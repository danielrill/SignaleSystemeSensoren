...
for i in range(len(logaWerteEingang)):
    a1 += (logaWerteEingang[i] - spannung_mean) * \
        (logaWerteAusgang[i] - distanz_mean)
...
parameteralog = np.exp([a * x + b for x in logaWerteEingang])

print(np.array(a1).shape)
print(np.array(parametera).shape)

ax[0].plot(parametera, logaWerteEingang)
ax[0].plot(logaWerteAusgang, logaWerteEingang, 'ro')
...
ax[1].plot(werte_x, werte_y, 'ro')
ax[1].plot(parameteralog, werte_y)
ax[1].grid(True)
...
for i in range(len(logaWerteEingang)):
    länge1 += (logaWerteEingang[i] - spannung_mean) * \
        (logaWerteAusgang[i] - distanz_mean)
...
flaecheDeltaV = sqrt(pow(input2 * deltaYL, 2) + pow(input1 * deltaYB, 2))
